...
### Auth Flow
OAuth 2.0 Authorization Code with PKCE. The app starts a transient HTTP listener on the fixed loopback redirect `http://127.0.0.1:4202/callback` for the duration of the login. Spotify's policy requires the IP literal `127.0.0.1`, not `localhost`, for loopback redirects. The fixed port matches the redirect URI registered in Spotify's developer dashboard for this app. Spotify redirects there with the auth code; the app exchanges it for tokens and shuts the listener down. No client secret is embedded in the binary. Refresh tokens live in the OS keyring, never on disk.
